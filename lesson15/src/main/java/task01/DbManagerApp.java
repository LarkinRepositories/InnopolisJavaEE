...
public class DbManagerApp {
    ...
    public void createTables() throws ClassNotFoundException, IllegalAccessException, InstantiationException, SQLException {
        ...
        ps.addBatch();
        SQL = "CREATE TABLE IF NOT EXISTS USER_ROLE(id INTEGER PRIMARY_KEY AUTO_INCREMENT, user_id INTEGER NOT NULL,  CONSTRAINT USER_id, role_id INT ADD CONSTRAINT THIS_ID_ROLE_ID FOREIGN KEY id REFERENCES USER_ROLE.id)";
        ps = connection.prepareStatement(SQL);
        ps.addBatch();
        ps.executeBatch();
    }
